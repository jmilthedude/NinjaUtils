...
public class BeaconVisualizerFeature extends Feature {
    ...
    private void clearAll(List<UUID> playerIds) {
        playerIds.forEach(uuid -> {
            ...
            ServerPlayerEntity player = ServerSetup.SERVER.getPlayerManager().getPlayer(uuid);
            if (player != null) {
    ...
    private void spawnParticles() {
        for (Map.Entry<UUID, VisualizerData> value : particles.entrySet()) {
            ServerPlayerEntity player = ServerSetup.SERVER.getPlayerManager().getPlayer(value.getKey());
